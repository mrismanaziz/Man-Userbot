...
@man_cmd(pattern="p(?: |$)(.*)")
async def _(event):
    await edit_or_reply(event, "**Assalamualaikum Koontoollllll**")
...
@man_cmd(pattern="k(?: |$)(.*)")
async def _(event):
    xx = await edit_or_reply(event, f"**HALLOO NGENTOOOTTTT SAYA {owner}**")
    sleep(2)
    await xx.edit("**LU SEMUA KONTOOOLLLLLL 🔥**")
